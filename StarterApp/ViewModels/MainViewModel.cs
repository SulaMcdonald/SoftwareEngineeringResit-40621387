...
public partial class MainViewModel : BaseViewModel
{
    ...
    /// @brief Navigates to the create conference page
    /// @details Relay command that navigates to the admin-only create conference page
    /// @return A task representing the asynchronous navigation operation
    [RelayCommand]
    private async Task NavigateToCreateConferenceAsync()
    {
        if (!IsAdmin)
        {
            await Application.Current.MainPage.DisplayAlert("Access Denied", "Only admins can create conferences.", "OK");
            return;
        }

        await _navigationService.NavigateToAsync("CreateConferencePage");
    }
}